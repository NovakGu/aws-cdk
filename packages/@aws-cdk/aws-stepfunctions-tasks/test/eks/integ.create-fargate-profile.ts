...
import { EksCreateFargateProfile, EksCreateCluster, FargateProfileSelector } from '../../lib';

/*
 * Create a state machine with a task state to create a cluster and create a fargate profile
 *
 * Stack verification steps:
 * The generated State Machine can be executed from the CLI (or Step Functions console)
 * and runs with an execution status of `Succeeded`.
 *
 * Note the cluster can take up to 15 minutes to create.
 *
 * Stack verification steps:
...
const chain = sfn.Chain.start(createClusterJob).next(createFargateProfileJob);
